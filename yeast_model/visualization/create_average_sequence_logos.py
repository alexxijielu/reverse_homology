...
if __name__ == "__main__":
    ...
    outdir = './avg_features-yeast/'        # Where to save sequence logos
    ...
    model.load_weights("../scer_idr_modell/1000_weights.h5")
    ...
    enc2 = LabelEncoder().fit(['G', 'A', 'L', 'M', 'F', 'W', 'K', 'Q', 'E', 'S', 'P', 'V', 'I', 'C', 'Y', 'H', 'R', 'N', 'D', 'T', '-'])
            ...
            for a in range(0, cells.shape[2]):
                ...
                if num_activations >= threshold:
                    if np.mean(curr_activation[:, a]) >= maximums[a] * percent_filter:
                        ...
                        if s == 0:
                            scer_sequence_names[a].append([name.split(".")[0], np.mean(curr_activation[:, a]),
                                                          rel_activation, ''.join(curr_seq)])
