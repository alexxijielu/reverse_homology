...
if __name__ == "__main__":
    ####### BASIC OPTIONS ################
    input_fasta = '../scer_idr_alignments/YAL001C_idr_3.fasta'
    weight_path = "../yeast_idr_model/1000_weights.h5"
    outdir = './YAL001C_idr_3-mutational-scanning/'
    z_score_file = "../yeast_idr_model/human_idr_features.txt"
    ######################################

    ####### ADVANCED OPTIONS #############
    species = ''    # Set to '' to just use the first sequence
    specify_positions = False
    start = 182
    end = 372
    remove_M = False

    autoselect_features = True
    minimum_significant = 5
    filters_of_interest = [482]
    ...
    for record in SeqIO.parse(input_fasta, "fasta"):
        if species == '':
            if rid == 0:
                ...
                # Set positions if not specified
                end = curr_len
                ...
                if save_webapp:
                    ...
                    for f in curr_filters:
                        ...
                        plt.close()
                        filter_averages.append(
                            [filter_type + " " + filter_idx, np.sum(curr_map[0:curr_len, :].T, axis=0)])
            ...
            if rid == 0:
                ...
                print("Species name is not unique - only outputted the first!")
