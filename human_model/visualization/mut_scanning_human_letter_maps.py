...
if __name__ == "__main__":
    ...
    idrs_of_interest = ['HUMAN06042_83to198']
    weight_path = "../human_idr_model/1000_weights.h5"
    outdir = './mutational_scanning/p27_heatmaps/'   # Directory to save maps into
    wscale = 1
    ...
    z_score_file = "../human_idr_model/human_conv3_features_1000_zscores.txt"
    ...
    # Create data loader
    datapath = '../hs_idr_alignments/'     # Point this to a directory of fasta IDR files
    n_features = 256
    seq_length = 256
    ...
    for i in range(0, ds.num_files):
        if ds.file_info[i]['name'].split(".")[0] in idrs:
            ...
            positions = idr_name.split("_")[1]
            start = positions.split("to")[0]
            end = positions.split("to")[1]
    ...
    for i in range(0, len(file_ids)):
        ...
        curr_matrix, curr_masks, _, curr_lengths, curr_species = ds.load_all_sequences_with_lengths_and_species(curr_file_id)
        species_index = [idx for idx, s in enumerate(curr_species) if "HUMAN" in s]
        if len(species_index) != 1:
            print("COULDN'T FIND HUMAN ENTRY FOR", curr_name)
        else:
